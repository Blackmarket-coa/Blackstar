...
|---|---|---|---|
| Gate 1: Staging End-to-End Order Flow Validation | ⚠️ Partially complete (rerun attempted on PHP 8.2 runtime, still environment-blocked) | `api/tests/Feature/StagingE2EValidationTest.php`, `reports/staging-e2e-validation.md`, `reports/staging-e2e-validation.json`, `reports/logs/setup-api-test-env-g12.log`, `reports/logs/preflight-g12-check.log` | Scenarios were re-run with `/root/.phpenv/versions/8.2snapshot/bin/php`; execution is still blocked in this container by missing `ext-sodium` and unreachable Packagist/GitHub endpoints when mirror/auth overrides are not supplied, leaving vendor bootstrap incomplete. |
| Gate 2: Vendor Visibility Constraints Verification | ⚠️ Partially complete (surface coverage complete; runtime execution attempted but blocked) | `api/tests/Feature/VendorVisibilityContractTest.php`, `reports/vendor-visibility-contract-coverage.md`, `reports/vendor-visibility-contract-coverage.json`, `reports/logs/VendorVisibilityContractTest.log`, `reports/logs/setup-api-test-env-g12.log` | Coverage artifacts are complete; direct suite execution remains blocked until dependency bootstrap succeeds in a PHP 8.2 runtime with `ext-sodium` and configured Composer mirror/auth overrides. |

...
| `POST /api/webhooks/freeblackmarket/retry` response envelope | API response | ❌ missing |
...
| `POST /api/shipment-board-listings/{id}/bids` response payload | API response | ❌ missing |
...
1. Add allowlist/denylist assertions for `POST /api/webhooks/freeblackmarket/retry`.
2. Add allowlist/denylist assertions for `POST /api/shipment-board-listings/{id}/bids`.
3. Add outbound contract test for `shipment.cancelled` emission.
